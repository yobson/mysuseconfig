...
disconnect() {
    nmcli con down id "$active" && send_notification "vpn-off" "Disconnected from $active" || send_notification "vpn-off" "<b>Error</b> disconnecting from $active!"
}
connect() {
    nmcli con up id "$chosen" && send_notification "vpn-on" "Now connected to $chosen" || send_notification "vpn-off" "<b>Error</b> connecting to $chosen!"
}
